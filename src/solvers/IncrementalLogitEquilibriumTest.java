...
public class IncrementalLogitEquilibriumTest implements GameSolver, GameOutcomePredictor {

  private String name;

  private QRESolver les = new QRESolver();

  private double startLambda = 25d;
  private double currentLambda = 25d;
...
//  private double[] samplingThresholds = {0.1d, 0.2d, 0.3d, 0.4d, 0.5d, 0.6d, 0.7d, 0.8d, 0.9d, 1d};
  ...
  public IncrementalLogitEquilibriumTest() {
    updateName();
  }
...
//    }
    ...
    while (gameObs.numObsLeft() > 0 &&
      ...
      if (eGame.getTotalNumSamples() % updateFrequency == 0 &&
          eGame.getTotalNumSamples() >= 15) {
        les.setLambda(currentLambda);
      ...
      if (!foundSample) {
        ...
        if (eGame.getTotalNumSamples() > 25 && currentLambda > 12.5d) {
          currentLambda = 12d;
          les.setLambda(currentLambda);
          samplingDistribution = les.predictOutcome(eGame);
        }

        if (eGame.getTotalNumSamples() > 35 && currentLambda > 8d) {
          currentLambda = 8d;
          les.setLambda(currentLambda);
          samplingDistribution = les.predictOutcome(eGame);
        }

        if (eGame.getTotalNumSamples() > 45 && currentLambda > 4d) {
          currentLambda = 4d;
          les.setLambda(currentLambda);
          samplingDistribution = les.predictOutcome(eGame);
        }
